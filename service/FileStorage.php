...
class FileStorage extends StorageAbstract
{
    ...
    public function get(array $options): array
    {
        ...
        if (($options['status']??-1) != -1) {
            foreach ($messages as $i => $message) {
                if (($message['status']??IPData::STATUS_ENABLED) != $options['status']) {
                    unset($messages[$i]);
                }
            }
        }
        ...
        foreach ($messages as $i => &$message) {
            ...
            if ($end_ts >= $current_time && $active_ts <= $current_time) {
                $message['state'] = IPData::STATE_ACTIVE;
            }
            else if ($end_ts < $current_time) {
                $message['state'] = IPData::STATE_EXPIRED;
            }
            else {
                $message['state'] = IPData::STATE_APPROACHING;
            }
        }
        unset($message);
    ...
    public function create(array $messages): array
    {
        ...
        $id++;
        $fields = array_flip(IPData::FIELDS);
